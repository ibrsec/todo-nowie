        ...
        {selectedTodo?.description.sort((a,b) => (a.stepName > b.stepName) ? 1 : ((b.stepName > a.stepName) ? -1 : 0)).map((item, i) => (
          <div key={i} className="flex items-center gap-2 border-b-2 pb-1 ">
            <input
              ...
              onChange={()=>putTask(path,selectedTodo?.id,{description:[...selectedTodo?.description.filter((steps,index)=> index!= i),{...item, completed:!item?.completed}]})}
            />
            <p className="flex-grow capitalize">{item?.stepName}</p>
            <span className="bg-violet-200 rounded-full p-1 cursor-pointer hover:bg-violet-100 active:bg-violet-300" onClick={()=>putTask(path,selectedTodo?.id,{description:[...selectedTodo?.description.filter((steps,index)=> index != i)]})}>
              <DeleteIcon />
            </span>
          </div>
        ))}
